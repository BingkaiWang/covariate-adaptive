...
for(iter in 1:sim_size){
  ...
  for (k in 1:num.strata){
    ...
    A[index]<- sample(c(rep(1,floor(pi*ns)),rep(0,ns-floor(pi*ns))))
  }
  ...
  lm.fit1 <- lm(Y~ . + 0 - W, data = sim.data)
  adj_strata <- lm.fit1$coefficients["A"]
  
  # adjusting for both strata and covariates
  lm.fit3 <- lm(Y~ 0 + ., data = sim.data)
  adj_both <- lm.fit3$coefficients["A"]
...
summary_table[2,2] <- CADE(Y = Y, A = A, Strata = I.S)[2]
summary_table[2,3] <- CADE(Y = Y, A = A, Strata = I.S, W = W)[2]

# estimate variance if covariate-adaptive design is ignored
summary_table[3,1] <- CADE(Y = Y, A = A)[2]
summary_table[3,2] <- CADE(Y = Y, A = A, W = I.S[,-1])[2]
summary_table[3,3] <- CADE(Y = Y, A = A, W = cbind(I.S[,-1], W))[2]

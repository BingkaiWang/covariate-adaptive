...
for(iter in 1:sim_size){
  ...
  glm.fit3 <- glm(Y~ 0 + ., data = sim.data, family = "gaussian")
...
summary_table[2,1] <- CADE(Y = Y, A = A, Strata = I.S, pi = pi, family = "gaussian")[2]
summary_table[2,2] <- CADE(Y = Y, A = A, Strata = I.S, pi = pi, W = W, family = "gaussian")[2]

# estimate variance if covariate-adaptive design is ignored
summary_table[3,1] <- CADE(Y = Y, A = A, pi = pi)[2]
summary_table[3,2] <- CADE(Y = Y, A = A, W = cbind(I.S[,-1], W), pi = pi, family = "gaussian")[2]
